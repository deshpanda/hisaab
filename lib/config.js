...
// ---------------------------------------------------------------------------
// EPF / EPS (projection defaults)
// ---------------------------------------------------------------------------
// Latest declared EPF interest rate. Interest accrues monthly on the running
// balance and is credited once at year end. Update on the annual pass.
export const EPF_INTEREST = { ratePct: 8.25, declaredFor: 'FY 2025-26' };

// Employer's 12% splits: EPS takes 8.33% of wage capped at the statutory
// ceiling; the remainder goes to the EPF account.
export const EPS = { rate: 0.0833, wageCeilingMonthly: 15000 };

// ---------------------------------------------------------------------------
// NPS — exit rules per PFRDA (Exits & Withdrawals) Amendment Regulations 2025
// (notified Dec 2025, in force for FY 2026-27). Non-government subscribers:
//   - normal exit (corpus > ₹12L): lump sum up to 80%, annuity at least 20%
//     (pre-Dec-2025 rule was 60/40)
//   - BUT only 60% of the corpus is tax-free under the carried-over 10(12A)
//     exemption — lump sum beyond 60% is taxable at slab (not yet aligned)
//   - corpus ≤ ₹8L at normal exit: 100% withdrawal allowed
// ---------------------------------------------------------------------------
export const NPS_RULES = {
  minAnnuityPct: 20,
  preDec2025MinAnnuityPct: 40,
  taxFreeCorpusPct: 60,
  smallCorpusFullWithdrawal: 800000,
};
...
export const GRATUITY = {
  eligibilityYears: 5,           // continuous service (waived on death/disability)
  roundUpMonthsThreshold: 6,     // final part-year counts only if it EXCEEDS 6 months
  taxExemptCap: 2000000,         // ₹20L lifetime exemption cap (private sector)
};
